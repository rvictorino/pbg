...
class IntroAction extends AbstractAction {
  ...
  play(){
    ...
    fabric.Image.fromURL(this.opponent.avatar, function(img){
      img.set(IMG_DEFAULT_OPT);
      img.scaleToWidth(canvas.getWidth()/5);
      img.set({
        left: - img.getWidth(),
        top: 0
      });
      canvas.add(img);
      img.animate('left', '+='+ canvas.getWidth());
    });
    fabric.Image.fromURL(this.player.avatar, function(img){
      img.set(IMG_DEFAULT_OPT);
      img.scaleToWidth(canvas.getWidth()/3);
      img.set({
        left: canvas.getWidth(),
        top: canvas.getHeight() - 350
      });
      canvas.add(img);
      img.animate('left', '-='+ canvas.getWidth());
    });
...
class TextAction extends AbstractAction {
    ...
    drawLetter(canvas, letter, line, pos, time){
      var letterImgPath = PbgHelper.getImagePathFromChar(letter);
...
class PbgHelper {
  ...
  static getImagePathFromChar(char){
    switch (char) {
        case "a":
